...
export function addObjectToArrayChange(tree: Tree, path: string): Change {
  ...
  return new InsertChange(path, routesListNode.getEnd(), routeToAdd);
}
...
export function addErrorRouteToRoutesArrayRule(path: string): Rule {
  return (tree: Tree) => {
    let change = addObjectToArrayChange(tree, path);
